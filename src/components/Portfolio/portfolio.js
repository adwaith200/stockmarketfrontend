...
export default class portfolio extends React.Component {
    ...
    fetch(lastPrice)
      ...
      .then(() => {
        ...
        if (change[parseInt(i)].split("")[1] === "-") {
          ...
          change[parseInt(i)] = `-INR${name.substr(2)}`;
        }
      });
  }
  ...
  handleStockSell(id) {
    ...
    fetch("http://localhost:5000/stockorder", {
    ...
    return (
      <div className="portfolio">
        ...
        <div className="portfolio__container">
          ...
          {this.state.loader1 === true && this.state.data.length > 0 && (
                ...
                {this.state.data.map((val, index) => {
                  return (
                    <tr key={index}>
                      <td>{val.symbol}</td>
                      <td>{val.shares}</td>
                      <td style={{color: parseInt(val.gainloss)>0?"#3ae885":"#F45385"}}>
                        {val.gainloss}
                      </td>
